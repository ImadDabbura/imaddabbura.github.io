...
categories: PyTorch
---
In Pytorch, you can define a layer with only the desired output size without specifying the input size. Later, when the layer is first called, Pytorch would check the input size from the input tensor and creates the required parameters. This is kinda of lazy definition of a layer. It is very helpful when we're not sure about the input size but know the output size. All the layerer that support **lazy** definitions are prefixed with `Lazy` such as `LazyLinear` and `LazyConv2d`.

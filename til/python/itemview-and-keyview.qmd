...
categories: Python
---
`dict.items()` and `dict.keys()` returns `ItemsView` and `KeysView` which are subclasses of `Set`.
Therefore, we can do some of the set operations on them such as intersection and union and the type of the object returned is a **Set**. Examples:

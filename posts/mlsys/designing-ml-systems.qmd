...
subtitle: "A reference guide to the design decisions between a trained model and a served prediction, deriving the trade-offs instead of asserting them"
...
Two things went into this guide: the canon, mostly Chip Huyen's [*Designing Machine Learning Systems*](https://www.oreilly.com/library/view/designing-machine-learning/9781098107956/) and Cathy Chen's [*Reliable Machine Learning*](https://www.oreilly.com/library/view/reliable-machine-learning/9781098106218/), which I am not trying to replace and will cite as I go; and a decade of building these systems across smart devices, healthcare, and enterprise AI, which is where the scars come from. What I have tried to add is the derivations. Where the canon hands you a rule of thumb, I show you where it comes from, because a rule you can derive is a rule you also know when to break. And where I expected one thing and the measurement said another, I say so.
...
::: {.callout-warning}
## This is the entire reason calibration matters
...
::: {.callout-note}
## A digression on feature stores, since I have been circling them for three sections
...
So: **hybrid**. Precompute the expensive, slow-moving features in batch (user embeddings, 90-day aggregates). Compute the cheap, fast-moving ones online (current session, current context). Join at request time. This is what most mature systems converge on, and it is why the [feature store digression](#sec-skew) exists.
...
Deployment is the beginning of the system's life, not the end of the project. That is why this is the longest section in the post.
...
::: {.callout-warning}
## Before you conclude the world moved, check that you did not move

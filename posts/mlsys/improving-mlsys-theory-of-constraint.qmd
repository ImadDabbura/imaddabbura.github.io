---
title: "Improving ML Systems with the Theory of Constraints"
subtitle: "A practitioner's guide to finding and fixing the one bottleneck that governs your system's performance."
image: "images/ml-toc.png"
title-block-banner: "images/ml-toc.png"
date: "2025-09-21"
...
![](images/network-anomaly-detection-toc.jpg)
...
In a serial pipeline (which is what most ML systems are), throughput equals the throughput of the slowest stage. If your feature extraction handles 30K records/sec and everything else handles 100K, the system does 30K. Making inference 10x faster OR doubling ingest capacity would still give us 30K records/sec. Only improving the bottleneck stage moves the number.

![**Figure 1:** Amdahl's Law applied to the pipeline. Making model inference (20% of runtime) 2x faster yields about 11% end-to-end improvement; making feature extraction (70%, the constraint) 2x faster yields about 35%. Only the constraint moves the system number.](images/amdahl-law-optimization.svg)
...
4. **Conflict → Innovation**: HOW do I resolve it? With an Innovation that captures the Pros of *both* the current approach and the new idea. The aim is all the Pros, but some tradeoffs may remain.
...
So why does this matter for ML specifically? Because ML pipelines are flow systems: ingest → features → inference → action. They have measurable stages with capacity limits. And they accumulate complexity over time: teams add features, models, and infrastructure without ever removing anything. This makes them natural candidates for constraint-based thinking. But ML teams rarely think this way, because they're trained to optimize *models*, not *systems*.

![**Figure 2:** The pipeline as a chain. Throughput is capped by the weakest link, Feature-Tier2 at 30K/s, no matter how much faster the other stages run.](images/ml-pipeline-weak-constraint.svg)

## Step 1: Define Your Goal and Find the Bottleneck

Before you can find the bottleneck, you need to define what success actually means in numbers (predefined metrics). And before you can fix the bottleneck, you need to know which stage is actually holding the system back. This step covers both.

### Set Your SLOs

"Detect anomalies," "reduce fraud," and "improve recommendations" aren't goals. Without measurable targets, every team member optimizes for a different thing, and you can't tell whether you're constrained by latency, precision, coverage, or something else entirely.
...
![**Figure 3:** SLOs cascade from a business goal down to technical metrics. Each layer's target (time, quality, coverage) constrains the ones below it.](images/slo-hierarchy.svg)
...
![**Figure 4:** The 24-hour validation test. Provisioning 3x compute for Feature-Tier2 alone lifted throughput from 30K to 90K rec/s and detection-within-5-minutes from 47% to 94%, confirming it was the true constraint.](images/before-after-dashboard.svg)
...
![**Figure 5:** Five Whys on the bottleneck, each answer validated with evidence. The chain bottoms out at an organizational root cause, not a technical one.](images/five-whys.svg)
...
![**Figure 6:** The visible constraint (Feature-Tier2 throughput) sits above technical, analytical, process, and cultural causes. Fixing the surface symptom without the causes beneath it lets the bottleneck return.](images/iceberg-visible-constraint.svg)
...
![**Figure 7:** The conflict as two competing requirements. A rich feature set buys 94% recall but only 30K rec/s; a minimal set handles 100K rec/s at 61% recall.](images/feature-extraction-conflict.svg)
...
![**Figure 8:** The alert-fatigue cycle. Lowering thresholds to catch more attacks floods analysts, who begin ignoring alerts, which causes more misses and another round of threshold lowering.](images/alert-fatigue.svg)
...
![**Figure 9:** The progressive architecture. Cheap triage clears 70% of traffic at wire speed; only uncertain records escalate to the more expensive tiers.](images/progressive-analysis-architecture.svg)
...
![**Figure 10:** Feature caching across tiers. Computing base features once and reusing them downstream turns a 255ms full recompute into 85ms at an 84% cache hit rate.](images/feature-dependency.svg)
...
![**Figure 11:** The 14-day minimum viable experiment, testing the single riskiest assumption before any production build.](images/mve-experiment-timeline.svg)
...
1. **Confidence calibration**: Initial triage was too conservative, with 32% of traffic passed to Tier-2 unnecessarily. The model lacked confidence on legitimate-but-unusual ports. Retraining with expanded examples achieved a 71% triage rate without missing attacks.
...
Most ML teams are stuck in optimization chaos: tuning components that don't govern system performance. The Theory of Constraints gives you a way out: find the one bottleneck that sets the ceiling, understand why it's stuck, and design an innovation that breaks the tradeoff instead of compromising on it.

*At any moment, only one thing limits your system: Find it & Fix it.*

If you take one thing from this post, make it this: before your next "optimization" sprint, build the constraint ledger. Measure every stage. Find the row with high utilization and a growing queue. That's where your effort yields the best results.

## References

- **Goldratt, E. M.** (1984). [The Goal: A Process of Ongoing Improvement](https://en.wikipedia.org/wiki/The_Goal_(novel)). *North River Press*.
...
- **Sommer, R. & Paxson, V.** (2010). [Outside the Closed World: On Using Machine Learning for Network Intrusion Detection](https://www.icir.org/robin/papers/oakland10-ml.pdf). *IEEE S&P*.
...
- **Ahmed, M., Mahmood, A., & Hu, J.** (2016). [A Survey of Network Anomaly Detection Techniques](https://doi.org/10.1016/j.jnca.2015.11.016). *Journal of Network and Computer Applications*, 60.
- **Kleppmann, M.** (2017). [Designing Data-Intensive Applications](https://dataintensive.net/). *O'Reilly*.
- **Polyzotis, N. et al.** (2018). [Data Lifecycle Challenges in Production Machine Learning](https://doi.org/10.1145/3299887.3299891). *SIGMOD Record*, 47(2).
- **Paleyes, A., Urma, R., & Lawrence, N.** (2022). [Challenges in Deploying Machine Learning: A Survey of Case Studies](https://arxiv.org/abs/2011.09926). *ACM Computing Surveys*, 55(6).

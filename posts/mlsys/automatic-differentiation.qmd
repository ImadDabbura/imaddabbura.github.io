...
subtitle: From dual numbers to backpropagation, uncover the inner workings of forward and reverse mode AD and the performance trade-offs that shape deep learning
date: 02-03-2024 
...
1. **Multiple Passes Required**: For a function with `n` inputs, Forward Mode AD requires `n` separate forward passes to compute all necessary derivatives. This characteristic makes it less efficient for functions with many input variables, which is often the case in deep learning applications as most of the loss functions are scalars.

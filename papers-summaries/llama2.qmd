...
date: 2023-10-05
description: "Open foundation and fine-tuned chat models with clever algorithmic tweaking focused on helpfulness and safety for dialogue."
categories:
status: evergreen
  - Scaling & Training
---
...
- **Methods**:
  - Models:
    ...
    - Use `KV` cache during inference. The task of LLM is **next token prediction**.
      At each step we're only interested in the prediction of the next token but
      we still need to give the previous tokens (context) that were already
      computed from before. So at step $10$, we would've already computed the
      attention scores for token_1 9 times and token_2 8 times. Also, we're 
      not interested in the attention scores for future tokens for causal inference since the attention score matrix is lower triangular. To summarize:
